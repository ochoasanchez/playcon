...
export function Scoreboard({ scoreboard, game } : { scoreboard: ScoreEntry[], game: 'memory' | 'trivia' }) {

    const sortedScores = game === 'memory' ? scoreboard.sort((a, b) => a.attributes.scoreValue - b.attributes.scoreValue) : scoreboard.sort((a, b) => b.attributes.scoreValue - a.attributes.scoreValue);
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="text-5xl text-center">
                            {sortedScores.map((score, index) => (
                                <tr key={score.id} className={`${getRowClassName(score.attributes.scoreValue)} border-b-2 border-white last:border-none`}>
                                    <td className="py-4">{index + 1}</td>
                                    <td className="py-4">{score.attributes.playerName}</td>
                                    <td className="py-4">{score.attributes.playerCompany}</td>
                                    <td className="py-4">{game === 'memory' ? (score.attributes.scoreValue/1000).toFixed(0) : score.attributes.scoreValue }</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
